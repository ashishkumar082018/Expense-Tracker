...
exports.generateReport = async (req, res) => {
  ...
  try {
    ...
    const p1 = Expense.findAll({
      ...
      where: {
        ...
        updatedAt: Sequelize.literal(
          `MONTH(updatedAt)=${new Date().getMonth() + 1}`
        ),
      },
    ...
    const p2 = Expense.findAll({
      ...
      where: {
        userId: id,
        updatedAt: Sequelize.literal(`YEAR(updatedAt)=${currentYear}`),
      },
...
exports.downloadReport = async (req, res, next) => {
  ...
  try {
    ...
    const fileName = `Expense${req.user.id}/${new Date()}.txt`;  // Defining file name
  
    // uploading file(report) to S3
    const fileUrl = await uploadToS3(stringifiedExpenses, fileName);
    await Report.create(
      { url: fileUrl, userId: req.user.id },
      { transaction: t }
    );
    await t.commit();
