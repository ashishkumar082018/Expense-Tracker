...
document.addEventListener('DOMContentLoaded', () => {
    ...
    axios.get(`http://localhost:3000/expense/getExpensePerPage?page=${page1.dataset.page}`, { headers: { "Authorization": token } })
        .then(result => {
            ...
            result.data.expenses.forEach(res => {
                ...
                editBtn.addEventListener("click", () => {
                    axios.delete(`http://localhost:3000/expense/delete-expense/${editBtn.parentElement.parentElement.id}`, { headers: { "Authorization": token } })
                        .then(res => {
                            if (expense >= income) {
                ...
                deleteBtn.addEventListener("click", () => {
                    axios.delete(`http://localhost:3000/expense/delete-expense/${deleteBtn.parentElement.parentElement.id}`, { headers: { "Authorization": token } })
                        .then(res => {
                            window.location.reload();
                        }).catch(err => console.log(err));
                });
...
page2.addEventListener('click', () => {
    axios.get(`http://localhost:3000/expense/getExpensePerPage?page=${page2.dataset.page}`, { headers: { "Authorization": token } })
        .then(result => {
            record.innerHTML = "";
            // if (result.data.expenses.length != 0) record.firstElementChild.lastElementChild.remove();
            result.data.expenses.forEach(res => {
