...
async function handleGetExpense(page) {
  ...
  try {
    ...
    if (getExpense.data.expenses.length != 0)
...
async function premiumGenerateReport() {
  try {
    ...
    if (
      ...
      monthlyExpense.firstElementChild.lastElementChild.innerText.trim() ==
        "No Data Available!"
    ) {
    ...
    if (
      ...
      yearlyExpense.firstElementChild.lastElementChild.innerText.trim() ==
        "No Data Available!"
    ) {
...
async function premiumShowAllReports() {
  try {
    ...
    if (
      ...
      allReports.firstElementChild.lastElementChild.innerText.trim() ==
        "No Data Available!"
    ) {
...
function handlePageNavDisable(pagination, pageNumber) {
  ...
  const lastPage =
    pagination.lastElementChild.previousElementSibling.firstElementChild
      .innerText;
  if (pageNumber > 1) {
